...
- **默认路径**：`%APPDATA%/MyHaruto/data/`（即 `C:\Users\<你>\AppData\Roaming\MyHaruto\data\`）
- **想换位置**：设置 → 数据位置 → 更改位置，选新目录后应用会把全部数据复制过去并校验，重启生效；复制或校验失败会自动回滚，原数据原地不动
- **迁移注意事项**：更改位置后旧目录数据保留一份作为保险，确认新位置运行正常后可手动删除；迁移前会自动做一次备份
- **备份建议**：定期备份整个数据目录（或在设置里查看当前路径后整目录拷贝）；应用每次启动也会自动滚动备份最近 7 份
