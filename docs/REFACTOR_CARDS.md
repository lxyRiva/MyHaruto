...
## 〇、执行卡强制模板（2026-09-11 用户定稿，所有执行卡必须遵守此结构）

**卡文件化（2026-09-11 用户令，立即执行）**：执行卡正文写入 docs/cards/<卡名>.md
独立文件；版本头三行缺一不可（版本/取代/生效）；改卡=改文件；开工=读文件；
审查=看 git diff docs/cards/；每次改卡附 3-5 行差异说明。v3.x/v5.1 全部作废。
**状态行（CARD_PROTOCOL §1）**：版本头后必须紧跟状态行——【状态】· 时间 ·
commit=hash/tag · 证据=报告路径；无证据=视为未完成；规划层独写，开发/测试只读。

一、开工锁：pwd / git log -1 / git status --short（现场实测，贴原文）
二、任务清单：每项独立编号不嵌套；每项写三要素——改什么（文件:行号）/改成什么（一句话）/为什么
三、禁止事项：写死（不许做 X/不许做 Y，逐条列）
四、验收标准：每条独立编号，含通过标准+反例（每条一个"应失败操作"证明规则生效）
五、报告格式：两段制（已实点+证据 / 未完成+清单[未做项/原因/手测步骤/通过标准]）
六、差异说明：必填 3-5 行（相对上一版改了什么）
冻结：未完成前不 commit。
...
| v1.38 | **阶段 2 执行卡 v3（用户 4 补丁后定稿，先审后转）**：补丁 1=②③改为贴当前代码原文+一句话结论（禁"已确认"三字；实测原文已取：TaskCard=pinnedGroupFirst(filter.sort(taskSort))✅ / ListTaskCard=filter.sort(taskSort) 无 pinnedGroupFirst✅）；补丁 2=施工前反问答案格式=消费点清单（文件:行号）+逐点核对结论，禁抄预期答案；补丁 3=新验收每条附截图/录屏+**每条附一个反例**（故意做应失败操作证明规则生效），禁纯文字"实点通过"；补丁 4=第零步明确按端口清 vite 孤儿（不依赖 concurrently -k，Windows 孙进程历史坑）+electron 优雅退出（alt+F4/WM_CLOSE）禁 taskkill |
| v1.39 | **执行卡 v3.1（用户 4 补丁）**：①删"横板子任务右键无置顶该组菜单项"反例（与 P1 已批"子任务入口保留"冲突；横板入口隐藏属 Pol9 未定案本卡不动）→替代反例=横板子任务置顶该组后**横板列表顺序不变**（前后截图）②A2/A3 措辞改"核对修复仍在 WIP，若被覆盖则重做"（上份报告已实点通过，非新任务）③开工锁与行号均**现场实测**（行号按注释锚 RF-B1 收尾 A1②/③ 定位，施工时以实测为准）④双链回归 8 点明确列出（置顶/日期编辑/番茄发起/检查事项/子任务折叠/优先级设置/删除/右键菜单） |
| v1.40 | **执行卡 v3.2 定稿（不再回审，直接开工）**：A2/A3 措辞落卡修正（第六条改"核对仍在 WIP，被覆盖则重做"）+第七条补横板反例（子任务置顶该组后横板列表顺序不变，前后截图）+明示"横板入口隐藏属 Pol9 本卡不动，本卡只验证横板不读 isPinnedGroup"；**第九条二次违例入册**（口头认领不落卡第二次，认领≠落地，落盘必须同次自查 diff） |
| v1.41 | **RF-B1 阶段 2 交付批复（用户：三处必须补，补完才 commit）**：①横板反例（置顶该组前后截图，唯一验证横板铁律的证据）开发必须补做 ②转用户手测清单逐项列出（未做项/原因/步骤/通过标准）③"环境冲突停手"理由不成立——未完成项真实原因必答+**UI 实点困难应对方案**（规划层裁定：本卡=开发与用户协商时间窗实点；**Playwright Electron 自动化基建登记 RF-Polish 池**[M5 前回归量爆炸前必须，新增 devDependency 需批准]）；补交报告两段制（已实点+证据/未完成+清单）；补交到位→规划层裁定三笔 commit；不到位→继续冻结 |
| v1.42 | **阶段 2 交付批复落册（用户：三处必须补，补交报告两段制）**：①横板反例 ②转用户手测清单 ③"环境冲突停手"理由不成立——①横板反例 ②转用户手测清单逐项列出 ③"环境冲突停手"理由不成立——未完成真实原因必答+应对方案（规划层已裁：本卡协商时间窗实点；Playwright Electron 自动化基建已入 RF-Polish 池[M5 前必须，devDependency 需批]）；补交到位→规划层裁定三笔 commit；不到位→继续冻结 |
| v1.43 | **补交卡批复修正两处（用户）**：①补交三首句重写——原"用户当时在审查"基于已撤回的错误信息（真 ②Playwright 池条目补 M5 定义（=PRD §5 里程碑「AI 聊天+视觉+人物主页」阶段）；**流程简化：补交报告直达用户手测不再经规划层审，补交到位→三笔 commit** |
| v1.44 | **补交卡批复+新 Agent 上岗补充任务（用户）**：①countOf 未落码实锤（useTaskSelectors:18-20 旧条件在位）→新 Agent 本轮补做并入第二笔 fix；**澄清责任**：v1.35"顺手清"vs 阶段 2 卡"归第二笔"规划层自相矛盾，开发无责新 Agent 尽责 ②Today 未来区口径=卡面笔误作废，开发行为正确不补测 ③ListTaskCard:8 死 import 实锤本轮顺手删（pinnedGroupFirst 已无消费/L87 纯 sort(taskSort)）④TaskCard:67 外层 sort 备案正确不处理 ⑤Recent7 旧直分=第二笔 fix 本卡不动；**新 Agent 三任务**：countOf 落码+死 import 删+v1.32 八条验收对照表（实点状态/证据索引/手测步骤/通过标准）；commit 边界更新（第二笔 fix=Recent7+countOf+死 import） |
| v1.46 | **上岗卡 v3（新 Agent 3 补丁+口径待拍板）**：①countOf 口径**待用户拍板**——甲=卡片数（今天区组卡数，tooltip「今天 N 个任务[含子任务 M 条]」双信息[推荐：用户原话"点进去看到几个"字面=卡数]）/乙=条目数（treeOf 成员总数含已完成）；乙实现成本与甲相同随时可换 ②对照表扩展：未完成清单并入四场景机械测试+双链回归 8 点，加"反例"列（v3.1 规矩）③追加三项：todayRoots 跨零点依赖（today 进 useMemo 依赖数组与否报告明说）/grep countOf 其他分支旧标记残留清点/L2 与 Today 一致必须 fresh dev 同帧截图禁文字声明 |
| v1.47 | **countOf 口径拍板=甲（卡片数）**：countOf('today')=今天区组卡数；tooltip「今天 N 个任务（含子任务 M 条）」双信息；乙弃用可随时切换。**新 Agent 上岗补充任务定稿落卡**（任务 1 共源结构性要求+任务 2 死 import+任务 3 对照表两态格式），随补交报告一并交付 |
| v1.53 | **台账机制+补测任务卡落盘**：①docs/cards/CARD_PROTOCOL.md（6 条硬规则：状态必附证据/权限分层/核实附命令输出/动态分级 L0-L3/用户审查兜底/误标纠错）②RF-B1 收尾补测任务卡产出（T1-T5：番茄<30s 门槛/递归归并补验/检查事项双链/披露①任务 11 核实/披露②0 分钟 session 清理）——两份待用户审 |
| v1.52 | **执行卡强制模板入册（用户定稿）**：六段结构（开工锁实测/任务清单三要素不嵌套/禁止事项写死/验收含反例/报告两段制/差异说明 3-5 行）——所有执行卡必须遵守；RF-B1 补交卡 v5 按模板重写发出 |
| v1.51 | **补交卡 v4 批复修正（用户）+定稿**：①v3 卡丢三处补交（countOf/死 import/八条对照表）必须补回，阶段 2 主施工改"核对在位"②⑧"横板无多余项"裁定=横板右键菜单无看板专属残留项（逐项核对：置顶该组父子一致保留/置顶今天保留/移动到对横板任务有语义保留），⑧验证=横板右键菜单九项齐全语义正确 ③④措辞改用户可见语言（检查项自动切模式刷新保留/删除后子孙消失+留痕） ④⑦改"开发已自测通过"（用户手测未确认） |
| v1.50 | **补交卡 v3 批复修正两处（用户）+卡定稿**：①折叠判定写死=DoneFoldSection 聚合成员（父任务 done+整组聚合）才算折叠不计；子任务自己 done、父未聚合仍计入；foldedOf 语义明文不许开发自解 ②双链 8 点逐条通过标准明文入卡（v2 卡"预填上述定义"系自指空转已修正）③UI 实点时间窗协商方式写明（开发交付报告中申请时段[预计耗时+可选时段]，用户确认后实点，不许默认转手测） |
| v1.49 | **补交卡批复六条（用户）+申辩核实结论**：①申辩核实=规划层发出的两张卡内容不同（第一张=阶段 2 执行卡 v3.2 含 A1/A4/A2A3 核对；第二张=上岗补充任务卡含 countOf/死import/对照表）——转发环节在用户手中，规划层无法核实实际粘贴副本；不升级指控，仅陈述：**若新 Agent 收到逐字相同的卡，问题在转发环节副本选择**②任务 1 打回两处确认（countOf 独立调 positionDateOf 违反结构性要求+tooltip 偏离双信息）③**countOf 折叠区裁定=不计**（"今天区所有显示的卡"自然推论：折叠区是独立区块非今天区子集；tooltip 明示口径）④对照表补证据索引列+双链 8 点逐条通过标准（规划层已补定义）⑤grep 证据须原文（路径+行号+代码行）禁总结 ⑥死 import 通过✅/04/05 截图待核/vite 强停披露维持可接受。冻结：任务 1/3 未重交前不 commit |
| v1.53 | **countOf 口径定死=X（用户二选一裁定）**：countOf('today')=今天区块根任务卡数（不含折叠区已完成根卡）——折叠区=收纳不需要做，计入虚高；与 v1.50 折叠判定"折叠区不计"衔接。v1.52 补差异说明入册 |
| v1.54 | **流程事故认领（用户：开发收到 v3.3 而非 v5）**：v5 卡（强制模板版）已发出且用户确认"卡正确可以开工"；**规划层随后又发 v3.3（删 countOf 验收 10，依据 v1.44 裁定）未显式声明取代关系**——双版本并存致开发收到 v3.3、用户以为 v5 在执行=版本管理事故主责规划层。**差异清单**：v3.3=v5 唯一删减项"验收 10 countOf 口径验证"（依据 v1.44 countOf 归第二笔 fix），其余内容一致→开发实现兼容 v5，验收 10 顺延第二笔 fix。**规约**：用户确认的卡如需修订必须 delta 修正+显式声明"取代 vX"，禁止平行版本 |
| v1.48 | **上岗卡 v3.2 修订（用户 3 补）**：①任务 1 删打架定义（treeOf 成员总数行删，唯一口径=甲组卡数，禁自选）②tooltip N/M 明文（N=组卡数/M=今天区全部后代数含已完成）③附带清点改"先列报告不擅自改"（'all'/tagId 口径未定不扩范围）+todayRoots 依赖链报告三项（today 来源/进依赖数组否/跨零点同步）④任务 3 逐点通过标准卡面预定义（四场景+双链 8 点，开发照填实测结果） |
| v1.55 | **执行卡版本事故认领（用户严正指出）**：v3.3 是阶段 2 主施工老卡（早已作废），不能取代 v5；**用户在 v5 上仅修订验收标准 10（countOf 口径 X 定死），该修订从未落卡**——规划层从未产出 v5 修订版，如实承认。v3.x 系列全部作废。v5.1 卡（v5 原文+验收 10 定稿）现已发出 |
| ⚠️ 勘误 | v1.32→v1.48 区间因多会话并行编辑出现行序错乱与局部重复（内容无损）；**版本顺序权威=git log（docs: v1.x commits）**，本表仅作内容速查。后续版本行按序追加于表尾，不再回填 |
...
| v1.54 | **T1-T5 补测报告批复落册+修复卡发出**：①T1 Toast 缺失（只静默丢弃无提示）——规划层建议乙 Toast 本轮必须做（30-50 行轻量组件），待用户拍板甲/乙 ②T2 独立 I=2 分来源追问（活数据 E 盘 66 任务/31 会话中无 I 任务——可能已被手测后清理，需开发在其实测环境查证）③T5 两处矛盾（T5 说 0 条/差异说明说清了 4 条孤儿）+越权删历史追问（授权仅限 0 分钟，孤儿 4 条含 8/24 历史遗留被越权删除）④次要追问 T3 双链覆盖范围/T4 机制说明 ⑤commit 三笔边界更新（fix 二轮 9 文件/feat 阶段 2+countOf+死import+T1 门槛/fix Recent7+groupPosition） |
| v1.57 | **修复卡 v2.0 批复三处硬遗漏（用户）+卡定稿**：①T5 方向修正：不 grep 0 分钟（重新核实≠追查），**追查已删 4 条**——从 backups/ 找回逐条列（taskId/时间/分钟数）；8/24 那条用户拍板恢复 or 不恢复；今后涉用户历史数据操作必须先问（新禁止事项）②T3 补答双链边界：右栏共享组件（TaskDetailContent）一次覆盖横板+看板详情展示，但**操作入口**（右键菜单/键盘导航/卡片勾选）分属两链需分别验证 ③禁止事项新增"禁止未获用户明确授权前对 focus-sessions/tasks 等已有历史数据做删除或修改；测试垃圾可清，历史数据一律先报后动" ④T1 触发机制定死：App 根级 Toast Context，usePomodoro 通过 context 消费（禁"方法或 state"二选一）⑤T2 简化：先查 I 记录已清则截图角标为准/未清则列明细 ⑥状态行日期 09-12/commit=未提交/版本注明独立文件不取代 v5.2 |
| v1.56 | **修复卡 v2 批复修正（用户：硬伤 2+漏洞 3+次要 5）**：①T1 Toast 范围定死：组件建 app 根级（全局 portal）、触发=completePomo 内部回调、**通用**（全项目复用非一次性）、标 [L2] 删"复用/新建"措辞（grep 零命中已确认无既有 Toast）②T2 追答命令修正（当前活数据已迁移，先查当前再翻历史）③T5 恢复步骤细化（先拍板要否恢复→整体回滚/手动提取二选一→不恢复归档）④验收反例补 ≥30s 不弹 Toast ⑤次要五项（问责机制与 §6 误标区分/差异说明指向/清场细则/时间窗/T1 归属） |

| v1.58 | **CARD_PROTOCOL 增补第七~九章（186 行）**：⑦规划层出卡提醒语句（6 条嵌入块，禁省略）⑧多轮迭代标注（同卡第 N 轮开发必须标注+滚动遗留清单+版本递增禁跳号）⑨规划层实时状态维护铁律（AGENT_STATE 每轮必刷/卡状态行每轮必刷/记忆每轮必做/python utf-8 统一写入，任一未做=违规） |
