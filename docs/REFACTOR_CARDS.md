...
| v1.35 | 阶段 2 手测反馈处置：A1 多组置顶**用户复测通过**；A4 混合场景确认（父参与取 min 未过期，组在 9/11 ✓ 零风险）；**问题 1 根因=Recent7View 漏接 groupPosition**（旧直分模型组被打散，修法待批=接入组单点）；**countOf 裁定=顺手 RF-B1 清**（去 isPinnedToday 条件，活跃逻辑会显示错）；**🔴 B1 排序语义·高优先级入 Pol 池顶**（前置依赖 RF-B1 commit，禁并回） |
