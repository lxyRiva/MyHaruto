...
| v1.45 | **上岗卡 v2（用户 3 批复）**：①tooltip 改"今天有 N 个任务"（中性，口径含已完成故"需要做"矛盾弃用）②**结构性钉子**：countOf('today') 必须复用 Today 页今天区分组结果（共用同一 selector/消费点，禁独立再算 groupPosition——原 bug 根子=countOf 与页面各算各的，只换算法双轨没死）③对照表两态格式：已实点=复验步骤（走一遍与证据一致即过）/未实点=首次手测步骤（A→B→C 看到 X 算过），不得同一格式 |
| v1.45 | **补交卡批复+新 Agent 上岗补充任务（用户）**：①countOf 未落码实锤（useTaskSelectors:18-20 旧条件在位）→本轮补做并入第二笔 fix；**澄清责任**：v1.35"顺手清"vs 阶段 2 卡"归第二笔"规划层自相矛盾，开发无责新 Agent 尽责 ②Today 未来区口径=卡面笔误作废，开发行为正确不补测 ③ListTaskCard:8 死 import 实锤（pinnedGroupFirst 无消费）本轮顺手删 ④TaskCard:67 外层 sort 备案正确不处理 ⑤Recent7 旧直分=第二笔 fix 本卡不动；**新 Agent 三任务**：countOf 落码+死 import 删+v1.32 八条验收对照表（实点状态/证据索引/手测步骤/通过标准）；commit 边界更新（第二笔 fix=Recent7+countOf+死 import） |
| v1.43 | **补交卡批复修正两处（用户）**：①补交三首句重写——原"用户当时在审查"基于已撤回的错误信息（真实=拖文件给 WB 时 zcode 占屏人机物理冲突，开发无责，质疑已撤回）；改为中性三选一如实说明（环境原因写具体场景如人机共用屏幕/鼠标冲突，不编造）②Playwright 池条目补 M5 定义（=PRD §5 里程碑「AI 聊天+视觉+人物主页」阶段）；**流程简化：补交报告直达用户手测不再经规划层审，补交到位→三笔 commit** |
| v1.42 | **阶段 2 交付批复落册（用户：三处必须补，补完才 commit；补交报告两段制）**：①横板反例截图开发补做（唯一横板铁律证据）②转用户手测清单逐项列出 ③"环境冲突停手"理由不成立——未完成真实原因必答+应对方案（规划层已裁：本卡协商时间窗实点；Playwright Electron 自动化基建已入 RF-Polish 池[M5 前必须，devDependency 需批]）；补交到位→规划层裁定三笔 commit；不到位→继续冻结 |
