...
四层结构（H1清单→H2标签→Section分组→任务）✅ 全部视图 ✅ → 收尾轮（右键统一✅【P2b 落地】/L1排序⏳【并入 RF-P3c】）→ **重构线进行中：P1 ✅ 60b0377、P2a ✅ 0754339、Fix1-3 ✅（2044e3f/4f83c9a/a744503）、P2b ✅ 38bafa5，下一卡 RF-P3a 全库搬移归位（bug 池 9 项在 RF-Fix4，P6b 后统一修）**（详见 docs/REFACTOR_CARDS.md）→ M5 AI聊天+视觉+人物主页（需智谱API Key）→ M6 AI定时行为 → V2 书影/旅行/看板拖拽/记忆衰减 → V3 小镇/Live2D/移动端
...
- 当前阶段：**RF-P2b 已提交（38bafa5，用户手动验收）；下一卡 RF-P3a（全库纯搬移归位 features/，diff 零逻辑变更）待派发；bug 池 9 项已入 RF-Fix4（P6b 后统一修，其中 R2/R3 疑似 Fix3 已修待对账）；RF-P7 设置中心已登记（Fix4 后，v1.0.0 收官随其后）**。后续循环以 docs/REFACTOR_CARDS.md 为准。
