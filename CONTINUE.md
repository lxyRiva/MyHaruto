...
四层结构（H1清单→H2标签→Section分组→任务）✅ 全部视图 ✅ → 收尾轮（右键统一✅/关联✅/死代码清理⏳【并入 RF-P2b】/L1排序⏳【并入 RF-P3c】）→ **重构线进行中：RF-P1 ✅（60b0377）、RF-P2a ✅（0754339），RF-Fix1 待开工**（详见 docs/REFACTOR_CARDS.md）→ M5 AI聊天+视觉+人物主页（需智谱API Key）→ M6 AI定时行为 → V2 书影/旅行/看板拖拽/记忆衰减 → V3 小镇/Live2D/移动端
...
- 当前阶段：**RF-P2a 已提交（0754339）；RF-Fix1（聚合语义树化+aiName 全局同步，两 bug 修复）待开工；P2b 等 Fix1 解锁；RF-P7 设置中心已登记（P6b 后，v1.0.0 收官随其后）**。后续循环以 docs/REFACTOR_CARDS.md 为准。
