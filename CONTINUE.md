...
四层结构（H1清单→H2标签→Section分组→任务）✅ 全部视图 ✅ → 收尾轮（右键统一⏳【RF-P2b 进行中】/L1排序⏳【并入 RF-P3c】）→ **重构线进行中：P1 ✅ 60b0377、P2a ✅ 0754339、Fix1 ✅ 2044e3f、Fix2 ✅ 4f83c9a、Fix3 ✅ a744503，RF-P2b 派发（bug 攒批至 RF-Fix4，P6b 后统一修）**（详见 docs/REFACTOR_CARDS.md）→ M5 AI聊天+视觉+人物主页（需智谱API Key）→ M6 AI定时行为 → V2 书影/旅行/看板拖拽/记忆衰减 → V3 小镇/Live2D/移动端
...
- 当前阶段：**RF-Fix3 已提交（a744503，用户手测通过）；RF-P2b（右栏统一+Today死代码+NewTaskBar，范围缩减版）已派发；bug 攒批策略生效——新 bug 收集至 RF-Fix4（P6b 后统一修）；测试交接已生效（P2b 起独立测试会话验收）；RF-P7 设置中心已登记（Fix4 后，v1.0.0 收官随其后）**。后续循环以 docs/REFACTOR_CARDS.md 为准。
